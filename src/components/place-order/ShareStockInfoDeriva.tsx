...
import type { ShareStock } from "../../types";
import {
  colorFix,
  getBgColorStock,
  numberFormat,
...
function ShareStockInfoDeriva({
  shareStock,
  loading,
}: {
  shareStock?: ShareStock | null;
  loading?: boolean;
}) {
  ...
  return (
    <div className="flex flex-col bg-skin-card rounded-xl gap-3 py-3">
      <div className="flex flex-row gap-4 items-center">
        {/* Ô giá chính */}
        <div
          className={`p-2 rounded-lg h-16 flex flex-col items-center justify-center gap-1 ${color} ${getBgColorStock(
            color,
          )}`}
        >
        ...
        <div className="grid grid-cols-3 gap-3 place-items-end">
          {[
            ...
            <div key={idx} className="grid gap-1 ml-4">
...
export default memo(ShareStockInfoDeriva);
