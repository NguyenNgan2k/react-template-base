...
const SelectAccount: React.FC<Props> = ({
  ...
  return (
    <div ref={showRef} className={`relative ${className}`}>
      <div
        className={`h-full flex items-center justify-between gap-1 px-3 rounded-2xl border cursor-pointer text-text-base text-xs ${disabled ? "opacity-60 cursor-not-allowed" : "hover:bg-neutral-800"
          } ${showSelect ? "border-yellow-500" : "border-neutral-700"} ${error ? "border! border-red-500!" : ""
          }`}
          ...
          <div className="max-h-60 overflow-y-auto custom-scrollbar p-2">
            {opts.map((item) => (
              <li
                ...
                className={`px-3 mt-1 py-2 rounded-md cursor-pointer flex items-center gap-3 text-text-base text-xs ${value === item.accCode
                    ? "bg-DTND-500"
