...
export function getMappedPriceKey(cellKey: string): string {
  if (cellKey.startsWith("volumeBuy")) {
    return cellKey.replace("volumeBuy", "priceBuy");
  }
  if (cellKey.startsWith("volumeSell")) {
    return cellKey.replace("volumeSell", "priceSell");
  }
  if (cellKey === "lastVolume") {
    return "lastPrice";
  }
  return cellKey; // còn lại thì không đổi
}

export function getChartColorDetail(
  lastPrice: number,
  c: number,
  f: number,
  r: number,
  ...
  if (isNaN(lastPrice) || isNaN(c) || isNaN(f) || isNaN(r)) return;

  if (lastPrice >= c) {
    return !type ? "#FF00FF" : "rgba(255, 0, 255, 0.05)"; // Giá trần
  } else if (lastPrice <= f) {
    return !type ? "#54B7FF" : "rgba(11, 160, 224, 0.1)"; // Giá sàn
  } else if (lastPrice > r) {
    return !type ? " #00BF52" : "rgba(0, 191, 82, 0.1)"; // Giá trên tham chiếu
  } else if (lastPrice === r) {
    return !type ? " #fdff12" : "rgba(180, 160, 15, 0.1)"; // Giá trên tham chiếu
  } else if (lastPrice < r) {
    return !type ? "#F23645" : "rgba(242, 54, 69, 0.1)"; // Giá dưới tham chiếu
  } else {
    return !type ? " #00BF52" : "rgba(0, 191, 82, 0.1)"; // Giá trên tham chiếu
  }
}
