...
export const selectFavorites = (state: RootState) =>
  state.priceBoard.data.favorites;

// Lấy thông tin chứng khoán đặt lệnh nhanh
export const selectQuickOrderSymbol = (state: RootState) =>
  state.priceBoard.data.quickOrderSymbol;

///Lấy thông tin chart bước giá
export const selectVolumeByPriceMap = (state: RootState) =>
  state.priceBoard.data.volumeByPrice;

export const selectVolumeByPriceStatus =
  (stockId: string) => (state: RootState) =>
    state.priceBoard.status.fetchVolumeByPrice[stockId] ?? {
      loading: false,
      error: null,
    };
