...
function RowComponentInday({
  ...
  return (
    ...
    >
      ...
      <div className={`w-[22%] text-right ${data?.split("|")?.[2]}`}>
        {data?.split("|")?.[1] && formatPrice(+data?.split("|")?.[1])}
      </div>
      <div className={`text-right w-[38%]  ${data?.split("|")?.[2]}`}>
...
function RowComponentForeign({
  ...
  return (
    ...
    >
      ...
      <div className={`w-[25%] text-right ${data?.split("|")?.[2]}`}>
        {data?.split("|")?.[1] && formatPrice(+data?.split("|")?.[1])}
      </div>
      ...
      {modeTable === "INDAY" ? (
        <div className="h-full flex flex-col">
          <div className="flex flex-row px-2 h-5">
            <div className="grow flex flex-row items-center gap-2 text-xs font-medium text-text-base">
              <div className="w-[15%]">Mã CK</div>
              <div className="w-[22%] text-right">Giá khớp</div>
              <div className="w-[38%] text-right">Thay đổi</div>
              <div className="w-[25%] text-right">KL</div>
            </div>
        ...
        <div className="h-full flex flex-col">
          <div className="flex flex-row px-2 h-5">
            <div className="grow flex flex-row items-center gap-2 text-xs font-medium text-text-base">
              <div className="w-[15%]">Mã CK</div>
              <div className="w-[25%] text-right">Giá</div>
              <div className="w-[30%] text-right">KL mua</div>
              <div className="w-[30%] text-right">KL bán</div>
            </div>
            <div className="shrink" style={{ width: size }} />
          </div>
          <div className="overflow-hidden h-[91px]">
