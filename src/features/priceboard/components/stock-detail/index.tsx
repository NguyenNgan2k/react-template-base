...
import ChartPriceStep from "@/components/stock/ChartPriceStep";
import ChartStockDetail from "@/components/stock/ChartStockDetail";
import CharWattingBuySell from "@/components/stock/ChartWattingBuySell";
...
export default function StockDetailModal({
  ...
  const targetSymbols =
    stock && stock.value ? `${stock.value}:G1:${stock.post_to}` : symbol;
        ...
        >
          ...
          >
            <div className="flex flex-col gap-6 p-2 rounded-xl border border-border bg-background-primary">
                ...
                <form className="h-full flex flex-row gap-4 items-center">
                  ...
                  <div className="flex flex-row items-center justify-center gap-3">
                    <div className="flex flex-row items-center justify-center gap-1">
                      {stock && (
                        <span className="text-xs font-normal text-text-base">
                ...
                <div className="flex justify-between">
                  <div className="flex gap-1 items-center justify-center">
                    ...
                    <div className="flex flex-row items-center justify-center">
                      <div
                        className={`text-color-down text-xs ${String(
                          snapshots[targetSymbols]?.orderBook?.["46"] || "",
                        ).split("|")[1]
                          }`}
                      >
                        {formatPrice(
                  ...
                  <div className="flex items-center justify-center pl-3">
                    ...
                    <div className="text-text-base pl-1 text-xs">
                      {formatVolPrice(
              ...
              {/* Dữ liệu phân tích */}
              <div className="grid grid-cols-6 gap-1 w-full h-[calc(var(--app-height)-320px)]">
                <div className="col-span-3 bg-surface rounded-md h-full p-1">
                  <ChartStockDetail symbol={symbol?.split(":")[0]} />
                </div>
                <div className="col-span-3 grid grid-cols-2 max-xl:grid-cols-1 gap-1 overflow-hidden">
                  <div
                    ref={containerRef}
                    ...
                    <div className="w-full h-full bg-surface rounded-md p-2">
                      <div className="flex flex-col min-h-0 w-full h-full ">
                        ...
                        <div className="flex-1">
                          <CharWattingBuySell symbol={targetSymbols} />
                        </div>
                      </div>
                    </div>

                    <div className="w-full h-full bg-surface rounded-md p-2">
                      <div className="flex flex-col gap-2 min-h-0 w-full h-full ">
                        <h2 className="text-sm font-medium text-text-title">
                          Bước giá
                        </h2>
                        <div className="flex-1">
                          <ChartPriceStep symbol={targetSymbols} />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-span-1 flex flex-col min-h-0 bg-surface rounded-md">
                    <h2 className="text-sm font-medium text-text-title p-2">
                      Chi tiết khớp lệnh
                    </h2>
