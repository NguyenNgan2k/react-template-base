import { ALL_COLUMNS_DERIVATIVE } from "@/configs";
...
export default function HeaderColumnsBase() {
  ...
  return (
    <div className="flex border-x border-b border-border divide-x divide-border w-full price-table-header">
      {columns.map((col) => {
        ...
        if (col.key === "mark" || col.key === "symbol") {
          return (
            <div
              key={col.key}
              className="h-14 grid place-items-center text-text-base text-xs font-medium select-none bg-gray-300/50"
              style={{ width: col.width }}
            >
              <div className="flex flex-col w-full">
                {/* Dòng 1: cột cha */}
                <div
                  className={`flex items-center justify-center ${hasChildren ? "border-b border-border h-7" : "h-14"
                    }`}
                >
                  {col.label}
                </div>
        ...
        return (
          ...
          >
            ...
            {/* Dòng 2: con */}
            {hasChildren && (
              <div className="flex divide-x divide-border w-full text-text-base text-xs font-medium select-none">
