...
function SortableRow({ symbol, snapshot, index, highlight }: SortableRowProps) {
  ...
  return (
    ...
    >
      {isOver && !isDragging && (
        <div className="absolute inset-x-0 -top-px h-0.5 bg-linear-to-r from-DTND-500 to-DTND-400 animate-pulse z-100" />
      )}
      <BodyTableBase
        symbol={symbol}
        snapshot={snapshot}
...
function PriceBoardBase({ boardId }: PriceBoardBaseProps) {
  ...
  return (
    <div
      ...
      ref={containerRef}
    >
      <div className="min-w-7xl flex flex-col h-full w-full">
